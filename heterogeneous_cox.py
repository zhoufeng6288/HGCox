...
#########################################################################################################
    ...
    def MF(self, num_gq, num_pre, num_iter, hp_tune=False):
        r"""
        ...
        for iteration in tqdm(range(num_iter), position = 0, leave = True):
            ...
            if hp_tune == True and (iteration+1)%2 == 0: # update hyperparameters every two iterations
                ...
                res = minimize(self.ELBO, initial_guss, args=(mean_fm,cov_fm), jac = gradient_hp, method='SLSQP', bounds=[(1e-4,None)]*((self.number_of_tasks+2)*self.number_of_latent), options={'maxiter': 2})
